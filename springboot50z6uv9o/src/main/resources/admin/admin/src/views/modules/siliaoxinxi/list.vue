<template>
	<div class="main-content" :style='{"padding":"20px 30px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","background":"#fff","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">饲料编号</label>
						<el-input v-model="searchForm.siliaobianhao" placeholder="饲料编号" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">饲料名称</label>
						<el-input v-model="searchForm.siliaomingcheng" placeholder="饲料名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="饲料种类" prop="siliaozhonglei">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">饲料种类</label>
						<el-select clearable v-model="searchForm.siliaozhonglei" placeholder="请选择饲料种类" >
							<el-option v-for="(item,index) in siliaozhongleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">供应商</label>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('siliaoxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('siliaoxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('siliaoxinxi','饲料库存')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column  :resizable='true' prop="siliaofengmian" width="200" label="饲料封面">
						<template slot-scope="scope">
							<div v-if="scope.row.siliaofengmian">
								<img v-if="scope.row.siliaofengmian.substring(0,4)=='http'&&scope.row.siliaofengmian.split(',w').length>1" :src="scope.row.siliaofengmian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.siliaofengmian)">
								<img v-else-if="scope.row.siliaofengmian.substring(0,4)=='http'" :src="scope.row.siliaofengmian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.siliaofengmian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.siliaofengmian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.siliaofengmian.split(',')[0])">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('siliaoxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('siliaoxinxi','投喂')" @click="siliaochukuCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								投喂
							</el-button>
							<el-button class="btn8" v-if="isAuth('siliaoxinxi','入库')" @click="siliaorukuCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								入库
							</el-button>
							<el-button class="edit" v-if=" isAuth('siliaoxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('siliaoxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<siliaochuku-cross-add-or-update v-if="siliaochukuCrossAddOrUpdateFlag" :parent="this" ref="siliaochukuCrossaddOrUpdate"></siliaochuku-cross-add-or-update>
		<siliaoruku-cross-add-or-update v-if="siliaorukuCrossAddOrUpdateFlag" :parent="this" ref="siliaorukuCrossaddOrUpdate"></siliaoruku-cross-add-or-update>
...
<script>
	...
	import siliaochukuCrossAddOrUpdate from "../siliaochuku/add-or-update";
	import siliaorukuCrossAddOrUpdate from "../siliaoruku/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				siliaochukuCrossAddOrUpdateFlag: false,
				siliaorukuCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			siliaochukuCrossAddOrUpdate,
			siliaorukuCrossAddOrUpdate,
		},
		methods: {
			imgPreView(url){
				this.previewImg = url
			...
			siliaochukuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.siliaochukuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','siliaoxinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.siliaochukuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.siliaochukuCrossaddOrUpdate.init(row.id,type);
				});
			},
			siliaorukuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.siliaorukuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','siliaoxinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.siliaorukuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.siliaorukuCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.siliaobianhao!='' && this.searchForm.siliaobianhao!=undefined){
					params['siliaobianhao'] = '%' + this.searchForm.siliaobianhao + '%'
				}
				...
				if(this.searchForm.siliaozhonglei!='' && this.searchForm.siliaozhonglei!=undefined){
					params['siliaozhonglei'] = this.searchForm.siliaozhonglei
				}
